...
class Episode11VisualsGenerator:
    """
    Generates high-contrast vertical diagrams (1000x1280) for Episode 11:
    - Card 1: State Machine & NavMesh Unstuck Conflict (Why enemy flies back)
    - Card 2: Engine Code Fix (Preventing Unstuck on ragdoll/falling dead actors)
    """
...
{html_content}
    ...
    def render_card1_navmesh_state(self, output_path: Path) -> Path:
        """
        Card 1: Simple 4-Step Chain:
        [Выбросили за текстуры] -> [Еще остались дела на поверхности] -> [Вернули назад] -> [Оставалось только умереть]
        """
        html = f"""
        <div style="
            width: {self.width}px;
        ...
        ">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #232730; padding-bottom: 20px;">
                <div>
                    <h2 style="font-size: 34px; font-weight: 800; color: #f8fafc; letter-spacing: -0.5px;">КАК СРАБОТАЛ БАГ? (4 ШАГА)</h2>
                    <p style="font-size: 21px; color: #94a3b8; margin-top: 4px;">Двойное совпадение правил игрового движка</p>
                </div>
            ...
            <div style="background: #1c1f26; border-left: 8px solid #38bdf8; border-radius: 18px; padding: 18px 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 25px; font-weight: 800; color: #38bdf8;">1. Выбросили за текстуры</span>
                ...
                <div style="font-size: 22px; color: #cbd5e1; line-height: 1.3;">
                    Монгола мощным пинком скинули с огромного утеса прямо в открытый океан.
                </div>
            </div>
            ...
            <div style="background: #1c1f26; border-left: 8px solid #f59e0b; border-radius: 18px; padding: 18px 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 25px; font-weight: 800; color: #fbbf24;">2. Ещё остались дела на поверхности</span>
                ...
                <div style="font-size: 22px; color: #cbd5e1; line-height: 1.3;">
                    Движок требует отыграть анимацию предсмертной агонии строго на ровной твердой земле.
                </div>
            </div>
            ...
            <!-- Step 3: Returned Back -->
            <div style="background: #1c1f26; border-left: 8px solid #ef4444; border-radius: 18px; padding: 18px 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 25px; font-weight: 800; color: #f87171;">3. Вернули назад</span>
                ...
                <div style="font-size: 22px; color: #cbd5e1; line-height: 1.3;">
                    Механизм защиты границ мира катапультирует NPC обратно на последнюю точку скалы.
                </div>
            </div>
            ...
            <div style="background: #1c1f26; border-left: 8px solid #22c55e; border-radius: 18px; padding: 18px 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 25px; font-weight: 800; color: #4ade80;">4. Оставалось только умереть</span>
                ...
                <div style="font-size: 22px; color: #cbd5e1; line-height: 1.3;">
                    ХП уже на нуле: приземлившись на скалу, бедолага спокойно завершает анимацию гибели.
                </div>
            </div>
            ...
            <div style="background: #0d0e11; border: 2px solid #232730; border-radius: 18px; padding: 16px 24px; display: flex; justify-content: space-between; align-items: center;">
                <div style="font-size: 22px; color: #94a3b8;">
                    Итог: <b style="color: #ffffff;">Защита границ спасла тело ради анимации смерти</b>
                </div>
    ...
    def render_card2_fix(self, output_path: Path) -> Path:
        """
        Card 2: Simplified Engine Code Fix Diff Card
        """
        html = f"""
        <div style="
            width: {self.width}px;
        ...
        ">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #232730; padding-bottom: 24px;">
                <div>
                    ...
                    <p style="font-size: 22px; color: #94a3b8; margin-top: 6px;">Если ХП на нуле — не возвращать на поверхность</p>
                </div>
            ...
            <div style="background: #0a0c10; border: 2.5px solid #1e293b; border-radius: 20px; padding: 28px 32px; font-family: Consolas, Monaco, monospace; font-size: 23px; line-height: 1.7;">
                ...
                <div style="background: rgba(239, 68, 68, 0.15); border-left: 6px solid #ef4444; padding: 12px 16px; margin-bottom: 16px; border-radius: 0 10px 10px 0; color: #fca5a5;">
                    ...
                    <span style="color: #94a3b8; font-size: 20px; margin-left: 32px;">// ➔ Катапультирует труп обратно на скалу!</span><br>
                ...
                <div style="background: rgba(34, 197, 94, 0.15); border-left: 6px solid #22c55e; padding: 12px 16px; border-radius: 0 10px 10px 0; color: #86efac;">
                    ...
                    <span style="color: #22c55e; font-weight: 900; margin-right: 12px;">[+]</span>}} // 🌊 Тело остается на дне и спокойно удаляется!
                </div>
            </div>

            <!-- Explanation Callout -->
            <div style="background: #1c1f26; border-left: 8px solid #38bdf8; border-radius: 18px; padding: 22px 28px;">
                <div style="font-size: 24px; font-weight: 800; color: #38bdf8; margin-bottom: 8px;">💡 Простое правило</div>
                <div style="font-size: 22px; color: #cbd5e1; line-height: 1.4;">
                    Если у персонажа закончились очки здоровья, система защиты границ не должна возвращать его на поверхность. Тело спокойно тонет в море без эффекта бумеранга.
                </div>
            </div>
            ...
            <div style="background: #0d0e11; border: 2px solid #232730; border-radius: 20px; padding: 20px 28px; display: flex; justify-content: space-between; align-items: center;">
                <div style="font-size: 23px; color: #94a3b8;">
                    Результат: <b style="color: #ffffff;">Никаких летающих мертвецов-бумерангов</b>
                </div>
