...
from src.core.episode5_en_visuals import Episode5EnVisualsGenerator
...
def build_episode_5_en():
    print("=" * 60)
    print("🚀 STARTING EPISODE #5 (EN) BUILD (Far Cry 5: Rocket Flatbed Truck)")
    print("=" * 60)

    # 1. Setup Output Structure
    print("\n[1/6] Preparing folder structure in output/5_en/...")
    mgr = EpisodeOutputManager("5_en")
    paths = mgr.get_paths("farcry5_truck_launch_en.mp4")
    ...
    meme_kosnulos_framed = mgr.visuals_dir / "meme_kosnulos_en.png"
    meme_shaking_gif = ASSETS_DIR / "memes" / "shaking.gif"
    meme_ragdoll_framed = mgr.visuals_dir / "meme_ragdoll_en.png"
    diag1_path = mgr.visuals_dir / "step1_overlap_en.png"
    diag2_path = mgr.visuals_dir / "step2_impulse_en.png"
    code_path = mgr.visuals_dir / "farcry5_fix_card_en.png"

    # Contextual Memes raw
    meme_kosnulos_raw = ASSETS_DIR / "memes" / "коснулось.jpg"
    meme_ragdoll_raw = ASSETS_DIR / "memes" / "ragdoll_en.png"
    ...
    print(f"   ✓ YouTube Library Ambient BGM: {bgm_path.name}")

    # 2. Scene-based TTS Blocks Definition
    print("\n[2/6] Generating English voiceover per scene block...")
    tts = TTSEngine(voice="en-US-ChristopherNeural", rate="+5%")

    scene_blocks_def = [
        {
            "id": "hook",
            "text": "When something touches your foot underwater. Meanwhile me...",
            "trigger_bgm": False
        },
        {
            "id": "investigation",
            "text": "But seriously, why did this truck panic over a dead cultist? Let's investigate!",
            "trigger_bgm": True
        },
        {
            "id": "ragdoll_intro",
            "text": "When a player carries a body, its physics are completely disabled. But the second you drop it, ragdoll physics instantly turn on.",
            "trigger_bgm": False
        },
        {
            "id": "scheme1",
            "text": "In this case, the player dropped the body too close to the truck. The corpse spawned clipped inside both the ground and the truck collision mesh at the same time.",
            "trigger_bgm": False
        },
        {
            "id": "scheme2",
            "text": "The Havok physics engine tried to push the body out of both objects at once! The massive separation impulse launched the 5-ton truck into orbit, kicking off Hope County's space program.",
            "trigger_bgm": False
        },
        {
            "id": "fix",
            "text": "To stop pickup trucks from launching to Mars, the developers just needed to clamp the maximum pushout force when an object is wedged in terrain.",
            "trigger_bgm": False
        },
        {
            "id": "outro",
            "text": "Have you ever sent vehicles into space in video games? Let me know in the comments and subscribe for more!",
            "trigger_bgm": False
        }
    ]
    ...
    print(f"   ✓ Total video duration: {total_duration:.2f} sec.")
    print(f"   ✓ Ambient start time (investigation block): {bgm_start_time:.2f} sec.")
    ...
    print("\n[3/6] Generating styled ASS subtitles...")
    ...
    # 4. Generate Visual Cards & Physics Diagrams
    print("\n[4/6] Generating English 2D CAD diagrams, meme frames, and code card...")
    vis_gen = Episode5EnVisualsGenerator(width=1000, height=560)
    vis_gen.generate_dog_meme_en(meme_kosnulos_raw, meme_kosnulos_framed)
    vis_gen.frame_meme(meme_ragdoll_raw, meme_ragdoll_framed, title_text="RAGDOLL PHYSICS IN GAMES")
    vis_gen.generate_fix_card_en(code_path)

    # CAD Physics Diagrams (New Engine Standard)
    renderer = PhysicsDiagramRenderer(width=1080, height=600)
    generate_scheme_farcry5_overlap(renderer, diag1_path, lang="en")
    generate_scheme_farcry5_launch(renderer, diag2_path, lang="en")
    ...
    print(f"\n[6/6] Compositing layers and audio tracks in FFmpeg (NVENC GPU)...")
    ...
    filter_chains = [
        # Meme 1: Dog leaping (Hook block, lower half y=860)
    ...
    # Preview frames
    print("\n📸 Generating preview frames in output/5_en/frames/...")
    frame_times = [
        hook_start + 1.0,
        inv_start + 1.0,
        rag_start + 1.0,
    ...
    print(f"\n✨ [SUCCESS] EPISODE #5 (EN) BUILT AND SAVED AT: {final_video}")
