...
def build_episode_8():
    print("=" * 60)
    print("🚀 НАЧАЛО СБОРКИ ВЫПУСКА №8 (Ghost of Tsushima: Зависший самурай на мосту)")
    print("=" * 60)

    # 1. Setup Output Structure
    print("\n[1/6] Подготовка файловой структуры в output/8/...")
    mgr = EpisodeOutputManager("8")
    ...
    card1_path = mgr.visuals_dir / "card1_detach.png"
    card2_path = mgr.visuals_dir / "card2_camera.png"
    card3_path = mgr.visuals_dir / "card3_fix.png"
    ...
    scene_blocks_def = [
        {
            "id": "hook",
            "text": "Дзин Сакай настолько преисполнился пути самурая, что остался медитировать на турнике, пока верный конь поскакал спасать Цусиму в одиночку!",
            "trigger_bgm": False
        },
        {
            "id": "investigation",
            "text": "Ghost of Tsushima славится идеальной кинематографичностью, но почему камера уехала вместе с конём, бросив главного героя? Начинаем расследование!",
            "trigger_bgm": True
        },
        {
            "id": "card1",
            "text": "При въезде на мост хитбокс самурая зацепил балку ворот. В игре нет триггера падения с седла, поэтому координаты персонажа намертво заблокировались в воздухе, разорвав связь с сокетом.",
            "trigger_bgm": False
        },
        {
            "id": "card2",
            "text": "А куда делся оператор? При верховой езде таргет камеры привязан к корневому объекту лошади. Конь автономно завершил переход, утянув за собой всю съёмочную группу!",
            "trigger_bgm": False
        },
        {
            "id": "card3_fix",
            "text": "Чтобы этого избежать, движок должен проверять наличие наездника в седле и переводить персонажа в регдолл при жёстком столкновении с препятствием.",
            "trigger_bgm": False
        },
        {
            "id": "outro",
            "text": "Конь забрал славу и оператора себе! Ставь лайк и подписывайся на канал, чтобы не пропускать новые разборы игровых багов!",
            "trigger_bgm": False
        }
    ]
    ...
    vis_gen.render_card1_detach(card1_path)
    vis_gen.render_card2_camera(card2_path)
    ...
    # 6: meme_cat (image)
    # 7: meme_scared (image)
    # 8: card1_detach.png (image)
    # 9: card2_camera.png (image)
    # 10: card3_fix.png (image)
    ...
    filter_chains = [
        ...
        # Card 1: Socket Detach (scale=1000:-1)
        ...
        # Card 2: Camera Rig Follow (scale=1000:-1)
        ...
        # Card 3: Bug Fix Code Card (scale=1000:-1)
    ...
    scenario_info = {
        ...
        "bug_title": "Зависший самурай на мосту (Разрыв иерархии сокетов)",
