...
def build_episode_6():
    print("=" * 60)
    print("🚀 НАЧАЛО СБОРКИ ВЫПУСКА №6 (Assetto Corsa: Чистовик с Кватернионами)")
    print("=" * 60)

    # 1. Setup Output Structure
    print("\n[1/6] Подготовка файловой структуры в output/6/...")
    mgr = EpisodeOutputManager("6")
    ...
    diag1a_path = mgr.visuals_dir / "scheme1a_normal.png"
    diag1b_path = mgr.visuals_dir / "scheme1b_glitch.png"
    diag2a_path = mgr.visuals_dir / "scheme2a_physics.png"
    diag2b_path = mgr.visuals_dir / "scheme2b_visual.png"
    code_path = mgr.visuals_dir / "scheme3_code_fix.png"

    # Contextual pure Memes
    meme_helicopter2_gif = ASSETS_DIR / "memes" / "helicopter2.gif"
    meme_wolf_img = ASSETS_DIR / "memes" / "wolf_captioned_ru.png"
    ...
    scene_blocks_def = [
        {
            "id": "hook",
            "text": "В Assetto Corsa болид мчит на полной скорости, но переднее колесо решило стать вертолётом!",
            "trigger_bgm": False
        },
        {
            "id": "investigation",
            "text": "При этом машина спокойно держит траекторию. Как такое возможно? Начинаем расследование!",
            "trigger_bgm": True
        },
        {
            "id": "scheme1",
            "text": "В играх вращение считается не по простым осям, а через четырёхмерный механизм кватернионов. Когда машина одновременно едет вперёд, рулит и крутит колесо — вычисления запутались и сложились в безумную ось!",
            "trigger_bgm": False
        },
        {
            "id": "scheme2",
            "text": "При этом физический луч сцепления работает отдельно от графики. Поэтому машина едет идеально, пока трёхмерная модель колеса танцует нижний брейк на скорости 3500 оборотов.",
            "trigger_bgm": False
        },
        {
            "id": "fix",
            "text": "Разработчикам нужно всего лишь правильно нормализовать кватернион и не смешивать рулевую ось с осью качения.",
            "trigger_bgm": False
        },
        {
            "id": "outro",
            "text": "Теперь болид снова на асфальте. А может, у него вчера просто был день ног? Пишите в комментарии и подписывайтесь на канал!",
            "trigger_bgm": False
        }
    ]
    ...
    # 4. Generate Visual Cards using CSS HTMLMotionRenderer
    print("\n[4/6] Генерация графических схем CSS Motion UI (Крупный шрифт)...")
    html_renderer = HTMLMotionRenderer(width=1000, height=540)
    html_renderer.render_scheme1a_normal(diag1a_path, lang="ru")
    html_renderer.render_scheme1b_glitch(diag1b_path, lang="ru")
    html_renderer.render_scheme2a_physics(diag2a_path, lang="ru")
    html_renderer.render_scheme2b_visual(diag2b_path, lang="ru")
    html_renderer.render_scheme3_code_fix(code_path, lang="ru")
    ...
    # 1: voice_final (audio)
    # 2: bgm (audio)
    # 3: sfx_helicopter (audio)
    # 4: sfx_transformers (audio)
    # 5: helicopter2.gif (animated GIF)
    # 6: wolf_captioned_ru.png (image)
    # 7: diag1a (image)
    # 8: diag1b (image)
    # 9: diag2a (image)
    # 10: diag2b (image)
    ...
    print("\n📸 Генерация превью-кадров в output/6/frames/...")
    frame_times = [
        hook_start + 1.0,
        inv_start + 1.0,
        sch1_start + 1.0,
    ...
    print("\n📝 Генерация метаданных и описания...")
    scenario_data = {
        "episode_id": "6",
        "game": "Assetto Corsa",
        "bug_title": "Баг вертолетного колеса (4D Quaternion Glitch)",
        "lang": "ru",
        "blocks": [
            {"id": "hook", "title": "Колесо-вертолет", "start": hook_start, "end": hook_end},
            {"id": "investigation", "title": "Расследование бага", "start": inv_start, "end": inv_end},
            {"id": "scheme1", "title": "Кватернион вращения", "start": sch1_start, "end": sch1_end},
            {"id": "scheme2", "title": "Физика vs Нижний брейк", "start": sch2_start, "end": sch2_end},
            {"id": "fix", "title": "Нормализация в коде", "start": fix_start, "end": fix_end},
            {"id": "outro", "title": "День ног", "start": outro_start, "end": outro_end}
        ],
        "metadata": {
            "tags": ["assettocorsa", "simracing", "racingglitch", "gamephysics", "gamedev", "quaternion", "shorts", "игры", "баги"]
        }
    }
    ...
    print(f"\n✨ [УСПЕХ] ВЫПУСК №6 (RU) СОБРАН И СОХРАНЕН В: {final_video}")
