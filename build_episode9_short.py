...
from src.core.episode9_visuals import Episode9VisualsGenerator
...
def build_episode_9():
    print("=" * 60)
    print("🚀 НАЧАЛО СБОРКИ ВЫПУСКА №9 (S.T.A.L.K.E.R. 2: Черная дыра видеопамяти)")
    print("=" * 60)

    # 1. Setup Output Structure
    print("\n[1/6] Подготовка файловой структуры в output/9/...")
    mgr = EpisodeOutputManager("9")
    ...
    card1_path = mgr.visuals_dir / "card1_camera_vertical.png"
    card2_path = mgr.visuals_dir / "card2_buffer_vertical.png"
    ...
    # 2. Scene-based TTS Blocks Definition (Simple & Clear Text)
    ...
    scene_blocks_def = [
        {
            "id": "hook",
            "text": "В Сталкер два нашли странную черную дыру. Сталкер кидает в неё болт, думая, что это новая аномалия Зоны...",
            "trigger_bgm": False
        },
        {
            "id": "investigation",
            "text": "Но на самом деле тут сломалась математика игры! Начинаем расследование!",
            "trigger_bgm": True
        },
        {
            "id": "card1",
            "text": "Как работает камера в играх? Она переводит 3D-мир в плоскую картинку экрана. Но в этой аномалии случилась ошибка — координаты улетели в бесконечность. Видеокарта решила: раз объекта на экране нет, рисовать там ничего не надо!",
            "trigger_bgm": False
        },
        {
            "id": "card2",
            "text": "А почему рука оставляет след? При взмахе видеокарта записала перчатку в память. Но когда рука ушла, фон позади не перерисовался! И старый кадр завис навсегда, как окно в Windows XP!",
            "trigger_bgm": False
        },
        {
            "id": "card3_fix",
            "text": "Лечится это просто: разработчики заставляют видеокарту принудительно очищать весь экран фоновым цветом перед каждым новым кадром.",
            "trigger_bgm": False
        },
        {
            "id": "outro",
            "text": "Болтом видеопамять не спасти! Ставь лайк и подписывайся на GameBug!",
            "trigger_bgm": False
        }
    ]
    ...
    # 4. Generate Visual Cards using Episode9VisualsGenerator (1000x1280)
    print("\n[4/6] Генерация вертикальных графических схем (Top 2/3, 1000x1280)...")
    vis_gen = Episode9VisualsGenerator(width=1000, height=1280)
    ...
    # 9: card1_camera_vertical.png (image, 1000x1280)
    # 10: card2_buffer_vertical.png (image, 1000x1280)
    # 11: card3_fix_vertical.png (image, 1000x1280)
    ...
    # sfx_zona in first 4s (vol 0.28), sfx_armatura when bolt is mentioned (vol 0.28), sfx_win_error on buffer (vol 0.32)
    ...
    scenario_info = {
        ...
        "bug_title": "Секретная аномалия видеопамяти (Screen Space Culling & Hall of Mirrors)",
